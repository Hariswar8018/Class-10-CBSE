        ...
        appBar: AppBar(
         ...
         floatingActionButton: FloatingActionButton(
          ...
          Navigator.push(
              ...
              child: Add_Blog (id: 'Updates10',), type: PageTransitionType.rightToLeft, duration: Duration(milliseconds: 800)
          ));
        },
        ) ,
        drawer : Df.buildDrawer(context, n),
        body : StreamBuilder(
          stream: FirebaseFirestore.instance.collection("Updates10").snapshots(),
  ...
  Widget build(BuildContext context){
    return Padding(
      ...
      child: InkWell(
        onLongPress: () async {
            print("hhh7788");
            await FirebaseFirestore.instance.collection("Updates10").doc(c.date).delete();
        },
        onTap: () async {
          String y = FirebaseAuth.instance.currentUser?.uid ?? "h";
          ...
          await FirebaseFirestore.instance.collection("Updates10").doc(c.date).update({
            "activity" : FieldValue.arrayUnion([y]),
          });
        },
        child: Container(
          height: 260,
